...
ligadas e vetores. Apesar de usarem mais memória, discutiremos detalhadamente as implementações 
usando listas ligadas. Isso pois são mais fáceis de entender, têm mesmo custo que as 
implementações usando vetores. Após entendidos os algoritmos de balanceamento, não deve ser difícil
implementar usando vetores.

\subsubsection{1-2 \SL}

Uma 1-2 \SL é uma \sl tal que 


\subsubsection{TB 1-2-3 \SL}
...
\begin{lstlisting}[caption=Algoritmo de inserção em Skip Lists determinísticas, language=C]
void skiplist_insert(SkipList *skiplist, int key) {
    int i;
    Cell *cell, *new_cell;
...
\begin{lstlisting}[caption=Algoritmo que resolve as violações das propriedades de Skip Lists provenientes de uma inserção, language=C]
...
\begin{lstlisting}[caption=Algoritmo que incrementa a altura de uma Skip List determinística, language=C]
